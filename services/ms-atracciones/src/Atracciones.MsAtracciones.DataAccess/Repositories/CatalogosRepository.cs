...
public sealed class CatalogosRepository : ICatalogosRepository
{
    ...
    private static ImagenDto Map(ImagenEntity x) => new(x.ImgGuid, x.ImgUrl, x.ImgDescripcion, x.ImgEstado, x.ImgFechaIngreso);

    public async Task<IReadOnlyList<DestinoDto>> ListDestinosActivosAsync(CancellationToken ct = default)
        => (await _db.Destinos.AsNoTracking().Where(x => x.DesEstado == 'A').OrderBy(x => x.DesNombre).ToListAsync(ct)).Select(Map).ToList();
    ...
    public async Task UpsertDestinoAsync(Guid guid, string nombre, string pais, string? imagenUrl, char estado, string usuario, string ip, CancellationToken ct = default)
    {
        ...
        if (entity is null)
        {
            ...
            {
                DesGuid = guid, DesNombre = nombre.Trim(), DesPais = pais.Trim(),
                DesImagenUrl = imagenUrl?.Trim(), DesEstado = estado,
                DesFechaIngreso = DateTime.UtcNow, DesUsuarioIngreso = usuario, DesIpIngreso = ip,
            });
        }
        else
        {
            entity.DesNombre = nombre.Trim(); entity.DesPais = pais.Trim();
            entity.DesImagenUrl = imagenUrl?.Trim(); entity.DesEstado = estado;
            entity.DesFechaMod = DateTime.UtcNow; entity.DesUsuarioMod = usuario; entity.DesIpMod = ip;
            if (estado == 'I') { entity.DesFechaEliminacion = DateTime.UtcNow; entity.DesUsuarioEliminacion = usuario; entity.DesIpEliminacion = ip; }
        }
        await _db.SaveChangesAsync(ct);
    }

    public async Task<IReadOnlyList<CategoriaDto>> ListCategoriasActivasAsync(CancellationToken ct = default)
        => (await _db.Categorias.AsNoTracking().Where(x => x.CatEstado == 'A').OrderBy(x => x.CatNombre).ToListAsync(ct)).Select(Map).ToList();
    ...
    public async Task UpsertCategoriaAsync(Guid guid, string nombre, Guid? parentGuid, char estado, string usuario, string ip, CancellationToken ct = default)
    {
        if (parentGuid.HasValue)
        {
            ...
            if (!parentOk) throw new InvalidOperationException($"Categoría padre {parentGuid} no encontrada.");
        }
        ...
        if (entity is null)
        {
            ...
            {
                CatGuid = guid, CatNombre = nombre.Trim(), CatParentGuid = parentGuid, CatEstado = estado,
                CatFechaIngreso = DateTime.UtcNow, CatUsuarioIngreso = usuario, CatIpIngreso = ip,
            });
        }
        else
        {
            entity.CatNombre = nombre.Trim(); entity.CatParentGuid = parentGuid; entity.CatEstado = estado;
            entity.CatFechaMod = DateTime.UtcNow; entity.CatUsuarioMod = usuario; entity.CatIpMod = ip;
            if (estado == 'I') { entity.CatFechaEliminacion = DateTime.UtcNow; entity.CatUsuarioEliminacion = usuario; entity.CatIpEliminacion = ip; }
        }
        await _db.SaveChangesAsync(ct);
    }

    public async Task<IReadOnlyList<IdiomaDto>> ListIdiomasActivosAsync(CancellationToken ct = default)
        => (await _db.Idiomas.AsNoTracking().Where(x => x.IdEstado == 'A').OrderBy(x => x.IdDescripcion).ToListAsync(ct)).Select(Map).ToList();
    ...
    public async Task UpsertIdiomaAsync(Guid guid, string descripcion, char estado, string usuario, string ip, CancellationToken ct = default)
    {
        ...
        if (entity is null)
        {
            ...
            {
                IdGuid = guid, IdDescripcion = descripcion.Trim(), IdEstado = estado,
                IdFechaIngreso = DateTime.UtcNow, IdUsuarioIngreso = usuario, IdIpIngreso = ip,
            });
        }
        else
        {
            entity.IdDescripcion = descripcion.Trim(); entity.IdEstado = estado;
            entity.IdFechaMod = DateTime.UtcNow; entity.IdUsuarioMod = usuario; entity.IdIpMod = ip;
            if (estado == 'I') { entity.IdFechaEliminacion = DateTime.UtcNow; entity.IdUsuarioEliminacion = usuario; entity.IdIpEliminacion = ip; }
        }
        await _db.SaveChangesAsync(ct);
    }

    public async Task<IReadOnlyList<IncluyeDto>> ListIncluyeActivosAsync(CancellationToken ct = default)
        => (await _db.Incluyes.AsNoTracking().Where(x => x.IncEstado == 'A').OrderBy(x => x.IncDescripcion).ToListAsync(ct)).Select(Map).ToList();
    ...
    public async Task UpsertIncluyeAsync(Guid guid, string descripcion, char estado, CancellationToken ct = default)
    {
        ...
        else { entity.IncDescripcion = descripcion.Trim(); entity.IncEstado = estado; }
        await _db.SaveChangesAsync(ct);
    }

    public async Task<IReadOnlyList<ImagenDto>> ListImagenesActivasAsync(CancellationToken ct = default)
        => (await _db.Imagenes.AsNoTracking().Where(x => x.ImgEstado == 'A').OrderByDescending(x => x.ImgFechaIngreso).ToListAsync(ct)).Select(Map).ToList();
    ...
    public async Task UpsertImagenAsync(Guid guid, string url, string? descripcion, char estado, string usuario, string ip, CancellationToken ct = default)
    {
        ...
        if (entity is null)
        {
            ...
            {
                ImgGuid = guid, ImgUrl = url.Trim(), ImgDescripcion = descripcion?.Trim(), ImgEstado = estado,
                ImgFechaIngreso = DateTime.UtcNow, ImgUsuarioIngreso = usuario, ImgIpIngreso = ip,
            });
        }
        else
        {
            entity.ImgUrl = url.Trim(); entity.ImgDescripcion = descripcion?.Trim(); entity.ImgEstado = estado;
            entity.ImgFechaMod = DateTime.UtcNow; entity.ImgUsuarioMod = usuario; entity.ImgIpMod = ip;
            if (estado == 'I') { entity.ImgFechaEliminacion = DateTime.UtcNow; entity.ImgUsuarioEliminacion = usuario; entity.ImgIpEliminacion = ip; }
        }
        await _db.SaveChangesAsync(ct);
    }
    ...
    public async Task<IReadOnlyList<DestinoDto>> GetDestinosByGuidsAsync(IEnumerable<Guid> guids, CancellationToken ct = default)
    {
        ...
        return (await _db.Destinos.AsNoTracking().Where(x => set.Contains(x.DesGuid) && x.DesEstado == 'A').ToListAsync(ct)).Select(Map).ToList();
    }
    ...
    public async Task<IReadOnlyList<CategoriaDto>> GetCategoriasByGuidsAsync(IEnumerable<Guid> guids, CancellationToken ct = default)
    {
        ...
        return (await _db.Categorias.AsNoTracking().Where(x => set.Contains(x.CatGuid) && x.CatEstado == 'A').ToListAsync(ct)).Select(Map).ToList();
    }
    ...
    public async Task<IReadOnlyList<IdiomaDto>> GetIdiomasByGuidsAsync(IEnumerable<Guid> guids, CancellationToken ct = default)
    {
        ...
        return (await _db.Idiomas.AsNoTracking().Where(x => set.Contains(x.IdGuid) && x.IdEstado == 'A').ToListAsync(ct)).Select(Map).ToList();
    }
    ...
    public async Task<IReadOnlyList<IncluyeDto>> GetIncluyeByGuidsAsync(IEnumerable<Guid> guids, CancellationToken ct = default)
    {
        ...
        return (await _db.Incluyes.AsNoTracking().Where(x => set.Contains(x.IncGuid) && x.IncEstado == 'A').ToListAsync(ct)).Select(Map).ToList();
    }
    ...
    public async Task<IReadOnlyList<ImagenDto>> GetImagenesByGuidsAsync(IEnumerable<Guid> guids, CancellationToken ct = default)
    {
        ...
        return (await _db.Imagenes.AsNoTracking().Where(x => set.Contains(x.ImgGuid) && x.ImgEstado == 'A').ToListAsync(ct)).Select(Map).ToList();
    }
}
