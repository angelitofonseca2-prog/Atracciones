...
namespace Atracciones.MsAtracciones.Api.Middleware;
...
public sealed class ExceptionHandlingMiddleware
{
    ...
    private static async Task HandleAsync(HttpContext context, Exception ex)
    {
        ...
        switch (ex)
        {
            case DomainValidationException ve:
            ...
            case ConflictException ce:
                ...
                body = new ApiErrorResponse { Status = 409, Error = "Conflicto", Details = new List<string> { ce.Message }, Path = context.Request.Path.ToString() };
                break;
            default:
